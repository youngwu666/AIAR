...
from model.model_utils import append_U_channel, compute_U_statistics, MSE, RMSE, relative_l2_error
...
def validate_epoch_decoder(dit, encoder, decoder, dev_loader, batch_parser, device, config, scaler):
    ...
    with torch.no_grad():
        for batch_idx, data in progress_bar:
            ...
            coords, coords_outputs, field = batch_parser.query_all(data)
            ...
            rmse = RMSE(scaler.inverse(u_pred), scaler.inverse(field))
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in progress_bar:
            ...
            if use_conditioning:
                ...
                indices = torch.randperm(H * W)[:num_sensor].to(device)
                row_indices = indices // W
                col_indices = indices % W

                sensor_value = field[:, :, row_indices, col_indices].permute(0, 2, 1)
            ...
            rmse = relative_l2_error(scaler.inverse(u_pred), scaler.inverse(field))
            ...
            if batch_idx == vis_batch:
                ...
                pred_field = append_U_channel(scaler.inverse(u_pred)).cpu().numpy()[vis_batch]
                true_field = append_U_channel(scaler.inverse(field)).cpu().numpy()[vis_batch]
                ...
                for i in range(C):
                    ...
                    axes[i, 1].scatter(col_indices.cpu().numpy(), row_indices.cpu().numpy(),  s=60, c='red', marker='*',
...
def test_diffusion_uncertainty(config, batch_parser, test_loader, scaler, device, num_samples=10):
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in progress_bar:
            ...
            if batch_idx == vis_batch:
                ...
                channel_names = ['Ux', 'Uy', 'p', 'k', 'epsilon', 'U']
                C_vis = len(channel_names)

                pred_field = u_mean.cpu().numpy()[vis_batch]
                true_field = u_true.cpu().numpy()[vis_batch]
                error_field = np.abs(true_field - pred_field)
                var_field = u_var.cpu().numpy()[vis_batch]

                fig, axes = plt.subplots(C_vis, 4, figsize=(4 * 4, C_vis * 3))

                for i in range(C_vis):
                    ...
                    im2 = axes[i, 1].imshow(pred_field[i, :, :], cmap='jet', origin='lower', vmin=vmin, vmax=vmax)
                    axes[i, 1].set_title(f'{channel_names[i]} - Mean Pred')
                    axes[i, 1].axis('off')
                    axes[i, 1].scatter(col_indices.cpu().numpy(), row_indices.cpu().numpy(), s=60, c='red', marker='*',
                                edgecolors='white', linewidths=0.5, zorder=10)
                    plt.colorbar(im2, ax=axes[i, 1])
                    ...
                    axes[i, 3].axis('off')
                    axes[i, 3].scatter(col_indices.cpu().numpy(), row_indices.cpu().numpy(), s=60, c='cyan', marker='*',
                                edgecolors='white', linewidths=0.5, zorder=10)
    ...
    print(f"Test l2 Relative Error: {np.mean(total_test_rmse):.4f} ± {np.std(total_test_rmse, ddof=0):.4f}")
...
def plot_ode_trajectory(traj, decoder, z_u, coords, scaler, H, W, channel_names, save_path, fps = 10):
    ...
    with torch.no_grad():
        for z in tqdm(traj, desc="预计算颜色范围", leave=False):
            ...
            u_pred = rearrange(u_pred, "b (h w) c -> b c h w", h=H, w=W)
            u_pred_scaled = append_U_channel(scaler.inverse(u_pred))
