...
def train_v(config, encoder, decoder, dit,
    ...
    for batch_idx, data in progress_bar:
        ...
        sensor_pos = torch.stack([y_sens, x_sens], dim = -1).expand(B, -1, -1).to(device)
        ...
        z_end = z_t + (1.0 - t[:, None, None]) * pred_v
        residual = (z_end - z_u).reshape(B, -1)
        ...
        loss = tangent.square().mean() + 0.05 * normal.square().mean()

        loss.backward()

        torch.nn.utils.clip_grad_norm_([dit.x_sens, dit.y_sens], 1.0)
...
def train_and_evaluate(config, device):
    ...
    latent_bank = torch.cat(latent_list, dim=0)[:64].contiguous()
    local_k = min(16, latent_bank.shape[0])
    ...
    for i in tqdm(range(latent_bank.shape[0]), desc="Precomputing local PCA", leave=False):
        ...
        basis_list.append(vh[:min(8, vh.shape[0])].contiguous())
