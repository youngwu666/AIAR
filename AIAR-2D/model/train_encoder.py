...
def validate(config, encoder, decoder, val_loader, batch_parser, device, scaler):
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in progress_bar:
            ...
            coords_outputs = coords_outputs.permute(0, 2, 1).unsqueeze(2)
...
def test(config, test_loader, scaler, device):
    ...
    batch_parser = BatchParser(config, H, W, device)
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in progress_bar:
            ...
            u_pred = u_net(decoder, latent_z, batch_coords)

            u_pred = rearrange(u_pred, "b (h w) c -> b c h w", h = H, w = W)

            rmse = relative_l2_error(scaler.inverse(u_pred), scaler.inverse(batch_inputs))

            total_test_rmse.extend(rmse.cpu().numpy())

            if batch_idx == vis_batch:

                true_field = append_U_channel(scaler.inverse(batch_inputs)).cpu().numpy()[vis_batch]
