...
class PatchEmbed(nn.Module):
    def __init__(self, patch_size=(16,16), emb_dim=768, channel = 5, use_norm=False):
        ...
        self.proj = nn.Conv2d(in_channels = channel,
...
class Encoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.H_patch, self.W_patch = config.patch_size
        self.H_img, self.W_img = config.grid_size
        self.emb_dim = config.emb_dim
        self.channel = config.channel

        self.patch_embed = PatchEmbed(config.patch_size,
                                      self.emb_dim,
                                      self.channel
                                      )

        self.pos_emb = nn.Parameter(
            get_2d_sincos_pos_embed(self.emb_dim,
...
class FourierEmbs(nn.Module):
    def __init__(self, embed_scale, embed_dim, in_dim=2):
